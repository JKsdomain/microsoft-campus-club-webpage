...
export async function POST(req: Request) {
  try {
      ...
      try {
        ...
        if (attempt) {
          resultData = {
            ...
            correctAnswersCount: attempt.correctCount || (attempt.questionSnapshot ? attempt.questionSnapshot.filter((q: any) => q.isCorrect).length : 0),
            incorrectAnswersCount: attempt.wrongCount || (attempt.questionSnapshot ? attempt.questionSnapshot.filter((q: any) => !q.isCorrect).length : 0),
            percentage: attempt.percentage || attempt.score,
            timestamp: attempt.completedAt ? new Date(attempt.completedAt).toISOString().replace("T", " ").substring(0, 19) : new Date().toISOString().replace("T", " ").substring(0, 19),
    ...
    if (!resultData && report) {
      resultData = {
        studentName: report.studentInfo?.name || report.username || "Student",
        studentEmail: report.studentInfo?.email || report.email || "student@mepcoeng.ac.in",
        department: report.studentInfo?.department || report.department,
        year: report.studentInfo?.year || report.year,
        section: report.studentInfo?.section || report.section,
        rollNumber: report.studentInfo?.rollNumber || report.rollNumber,
        testType: report.testType || "Test Assessment",
